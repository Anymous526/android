	...
	public void onCreate(Bundle savedInstanceState) {
		...
		btn_save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String postion = et_position.getText().toString();
			...
			long success =	DBHelper.getInstance(AddNew.this).save(user);
			if(success != -1){
				Toast.makeText(AddNew.this, "添加成功", Toast.LENGTH_LONG).show();
				//Result为1的时候代表增加用户成功,返回到主页面
				setResult(1);
				finish();
			}else{
				Toast.makeText(AddNew.this, "插入出错,请重新操作", Toast.LENGTH_LONG).show();
				//Result为2的时候代表增加用户失败
				setResult(2);
				finish();
			}
		...
		btn_rerutn.setOnClickListener(new OnClickListener(){
	...
	private void initImageChooseDialog() {
		...
		builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				btn_img.setImageResource(images[imagePostion%images.length]);
